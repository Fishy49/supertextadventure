  ...
  module Handlers
    class CombatHandler < BaseHandler
      ...
      def handle_attack_in_combat
        ...
        if combat["creature_health"] <= 0
          return handle_creature_defeat(creature_id, creature_def)
        end
        ...
        if current_health <= 0
          return handle_player_death(creature_def)
        end
      ...
      def handle_defend
        ...
        lines << "You take #{creature_damage} damage. Your health: #{current_health}/#{player_state['max_health'] || 10}"
