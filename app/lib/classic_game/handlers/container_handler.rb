  ...
  module Handlers
    class ContainerHandler < BaseHandler
      ...
      def handle_open(target)
        ...
        unless item_accessible?(container_id)
          return failure("You don't see that here.")
        end

        # Check if it's a container
        unless container_def["is_container"]
          return failure("You can't open that.")
        end

        # Check if it's already open
        if game.container_open?(container_id)
          return failure("It's already open.")
        end

        # Check if it's locked
        if container_def["locked"]
          unlock_item = container_def["unlock_item"]
          if unlock_item && has_item?(unlock_item)
      ...
      def handle_close(target)
        ...
        unless game.container_open?(container_id)
          return failure("It's already closed.")
        end
